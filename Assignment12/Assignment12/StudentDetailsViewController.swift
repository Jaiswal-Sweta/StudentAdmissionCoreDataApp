...
class StudentDetailsViewController: UIViewController {
    ...
    private let ChangePasswordButton : UIButton = {
        ...
        button.addTarget(self, action: #selector(OnChangedPasswordClicked), for: .touchUpInside)
    ...
    override func viewDidLoad() {
        ...
        s = CoreDataHandler.shared.fetchStudentSpecific(Spid: UserDefaults.standard.integer(forKey: "SpidUser"))
            NameTextfield.text = s[0].name  // set name
            //set gender segment
            if s[0].gender == "MALE" {
                GenderSegment.selectedSegmentIndex = 0
            } else if s[0].gender == "FEMALE" {
                GenderSegment.selectedSegmentIndex = 1
            }else if s[0].gender == "OTHER" {
                GenderSegment.selectedSegmentIndex = 2
            }
            
            ClassTextfield.text = s[0].sclass
            
            //set date picker
        BirthDatePicker.setDate(s[0].dob!, animated: true)
    ...
    @objc func OnChangedPasswordClicked()
    {
