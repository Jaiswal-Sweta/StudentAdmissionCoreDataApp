...
class LoginViewController: UIViewController {
    ...
    private let UsernameTextfield: UITextField = {
        let textField = UITextField()
        textField.attributedPlaceholder =
            NSAttributedString(string: "Enter Username or Spid", attributes: [NSAttributedString.Key.foregroundColor: UIColor.black])
        textField.text = ""
        textField.textColor = .black
        textField.textAlignment = .center
        textField.backgroundColor = .clear
    ...
    @objc func OnLoginClicked()
    {
        ...
        {
            UserDefaults.standard.setValue(UsernameTextfield.text, forKey: "SpidUser")
            let n = Admin_DashboardViewController()
            navigationController?.pushViewController(n, animated: true)
        }
        else
        {
            let StudentArray = CoreDataHandler.shared.CheckUser(spid: Int(UsernameTextfield.text!)!, password: PasswordTextfield.text!)
            
            if StudentArray == [Student]()
            {
            ...
            {
                UserDefaults.standard.setValue(self.UsernameTextfield.text, forKey: "SpidUser")
